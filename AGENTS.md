...
- **测试**: 287 tests / 21 classes total (`./gradlew :core:test test`)
- **版本**: 0.7.0
...
./gradlew pureTest      # alias for :core:test; 78 core pure JVM tests, no IntelliJ Platform test runtime
./gradlew test          # 209 platform/integration tests / 10 classes
...
- 287 tests / 21 classes 覆盖：78 个 core pure JVM 测试 + 209 个平台/集成测试，含真实 Git 集成、取消、rollback、derive 安全、通知决策、stash+rollback、50 子模块调用预算。
