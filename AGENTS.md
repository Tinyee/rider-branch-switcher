...
- **测试**: 306 tests / 21 classes, `./gradlew test`
- **版本**: 0.7.0
...
./gradlew test          # 306 tests / 21 classes
...
- 306 tests / 21 classes 覆盖：真实 Git 集成、取消、rollback、derive 安全、通知决策、stash+rollback、50 子模块调用预算。
...
24. **复审结论必须主动写回共享文档。** 只要用户要求“审查”“复审”“再审”“看改动”且当前已有 `docs/ai-review-current.md` 上下文，必须在同一轮把结论、延期/接受理由、验证命令同步更新到该文档；不要等用户再次提醒“写入文档”。
