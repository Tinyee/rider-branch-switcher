...
- **测试**: 279 tests / 27 classes (14 core + 13 platform)
- **版本**: 0.7.0
...
├── git/          GitOps (CLI 实现)
├── service/      BranchSwitcherService, PresetRepository
...
- 279 tests / 27 classes 覆盖：139 个 core pure JVM 测试 + 140 个平台/集成测试（含 6 个 Kotest 属性测试），含真实 Git 集成、取消、rollback、derive 安全、通知决策、stash+rollback、50 子模块调用预算。
...
7. **加了有逻辑的 getter 后，确认内部代码没有绕过它用裸字段。** getter 承担校验、权限或规范化逻辑时，新增后要 `grep` 同名裸字段并确认所有内部引用点。
