...
- **测试**: 287 tests / 28 classes (11 core + 14 platform, +TestHelpers)
- **版本**: 0.7.0
...
├── switch/       BranchNameRules, DeriveNotification, DeriveResult
...
./gradlew pureTest      # alias for :core:test; 135 core pure JVM tests (15 classes)
./gradlew test          # 146 platform/integration JUnit tests + 6 Kotest property tests (13 classes)
...
- 287 tests / 28 classes 覆盖：135 个 core pure JVM 测试 + 152 个平台/集成测试（含 6 个 Kotest 属性测试），含真实 Git 集成、取消、rollback、derive 安全、通知决策、stash+rollback、50 子模块调用预算。
