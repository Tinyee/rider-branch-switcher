...
- **测试**: 287 tests / 25 classes (11 core + 14 platform, +TestHelpers)
- **版本**: 0.7.0
...
./gradlew pureTest      # alias for :core:test; 78 core pure JVM tests (11 classes)
./gradlew test          # 203 platform/integration JUnit tests + 6 Kotest property tests (14 classes)
...
- 287 tests / 25 classes 覆盖：78 个 core pure JVM 测试 + 209 个平台/集成测试（含 6 个 Kotest 属性测试），含真实 Git 集成、取消、rollback、derive 安全、通知决策、stash+rollback、50 子模块调用预算。
