...
- **冲突文件处理**:预览会标出 checkout 将要覆盖的未跟踪文件,并提供丢弃选项——全部丢弃,或只丢弃目标分支已有受跟踪版本的 `.meta` 文件。
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| 未提交改动 | 暂存改动后切换 | 暂存并恢复改动——这是安全默认,未提交的改动会被保留并在切换后恢复;或跳过脏仓库,或不暂存直接尝试 checkout。成功恢复后会自动尝试删除该 stash 条目;若恢复失败、中断,或恢复后的删除失败,条目会作为手动恢复备份保留在 `git stash list` 里。 |
...
- **仅丢弃 .meta 文件**——保留其他所有文件。被丢弃的每个 `.meta` 文件都已被目标分支跟踪,将由目标分支的版本替换。
