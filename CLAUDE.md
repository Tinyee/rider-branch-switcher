...
以下记录的是真实过失，但没有对应的 grep/命令/门禁可以自动化拦截。知道就行，不当成可执行的规则。

- **在 A 处修了 bug 没扫 B/C/D 处。** 缺 try/catch、缺 cancel 检查、缺 operation 生命周期——同一个 bug 在 3+ 个文件里各犯了一遍。
- **只修实例没修模式。** 同一类 bug 被审查抓了 5 轮。
- **为了让测试绿而放松安全检查。** tri-state probe 初期设计成 fail-open 就为了让测试过。
- **审查指出的验证缺口加注释承认就当修完。** checkQuickCheck 被指出"无法区分规则命中与基础设施失败"，我只加了一句注释；结果 `gradlew.bat` 路径错误导致全假阳性，quickCheck 一次都没跑到。
- **取消操作跳过回滚。** 取消了也得在新 operation 里把已改的仓库恢复。
- **顺手改和当前任务无关的代码。** "来都来了"导致 scope 失控。
- **补测试是为了凑数不是防回归。** PresetOverrides 结构相等性测试——生产代码全删了它照样绿。
...
- **静默吞异常。** `catch (_: Exception) {}`——`TooGenericExceptionCaught` 在 detekt 中为 `active: false`，未启用。这条靠代码审查。
