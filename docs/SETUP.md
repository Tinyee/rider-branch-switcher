...
```bash
./gradlew :core:test    # 67 个 core 纯 JVM 用例（model / JSON / import / settings rules / preset file IO / GitResult / Swing layout rules / switch preview），不启动 IntelliJ Platform 测试运行时
./gradlew test          # 220 个平台/集成用例（mock GitClient / 真实 git 仓库 / UI 规则 / Kotest 属性测试）
```
